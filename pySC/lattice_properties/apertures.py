...
def SCdynamicAperture(RING, dE, bounds=np.array([0, 1e-3]), nturns=1000, thetas=np.linspace(0, 2 * np.pi, 16),
    ...
    if launchOnOrbit:
        if useOrbit6:
            ZCO = at_wrapper.findorbit6(RING)
        else:
            tmp = at_wrapper.findorbit4(RING, 0)
            if ~np.isnan(tmp[0]):
                ZCO[0:4] = tmp
    ZCO[5] = ZCO[5] + dE
    ...
    if centerOnOrbit:
        if useOrbit6:
            tmp = at_wrapper.findorbit6(RING)
        else:
            tmp = at_wrapper.findorbit4(RING, 0)
...
def SCmomentumAperture(RING, REFPTS, inibounds, nturns=1000, accuracy=1e-4, stepsize=1e-3, plot=0):
    ...
    ZCOs = at_wrapper.findorbit6(RING, REFPTS)
    ...
    dbounds = np.array([dboundHI, dboundLO]).T
    if plot:
        spos = at_wrapper.findspos(RING)[REFPTS]
        plt.figure(81222)
        plt.clf()
