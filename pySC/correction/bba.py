...
def SCBBA(SC, bpm_ords, mag_ords, **kwargs):
    ...
    for jBPM in range(bpm_ords.shape[1]): # jBPM: Index of BPM adjacent to magnet for BBA
        for nDim in range(bpm_ords.shape[0]):
            ...
            if par.mode == 'ORB':
                BPMpos, tmpTra = _data_measurement(SC, mOrd, BPMind, jBPM, nDim, par, _get_orbit_bump(SC, mOrd, bpm_ords[nDim,jBPM], nDim, par))
            else:
            ...
            OffsetChange, error_flags[nDim,jBPM] = _data_evaluation(SC, bpm_ords, jBPM, BPMpos, tmpTra, nDim, mOrd, par)
            SC = SC0
...
def _data_measurement(SC, mOrd, BPMind, jBPM, nDim, par, varargin):
    ...
    for nQ, setpointQ in enumerate(par.magSPvec[nDim,jBPM]):
        ...
        for nKick in range(nMsteps):
            ...
            tmpTra[nKick,nQ,:] = B[measDim, : ] if par.mode == 'ORB' else B[measDim, (BPMind+1):(BPMind+par.maxNumOfDownstreamBPMs) ]
...
def _data_evaluation(SC, BPMords, jBPM, BPMpos, tmpTra, nDim, mOrd, par):
    if par.plotLines:
        fig, ax = plt.subplots(num=56, projection="3d")
...
def _scale_injection_to_reach_bpm(SC, BPMind, nDim, kickVec0):  # nDim 0 or 1, for  hor, ver
    initialZ0 = SC.INJ.Z0.copy()
...
def plotBBAResults(SC,initOffsetErrors,errorFlags,jBPM,BPMords,magOrds):
    ...
    plt.ylabel(r'Offsets change [$\mu$m]')
    plt.xlabel('Index of BPM')
    plt.set(gca,'XLim',[1 length(SC.ORD.BPM)],'box','on')
