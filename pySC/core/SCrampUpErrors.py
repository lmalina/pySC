...
def SCrampUpErrors(SC,nStepsRamp=10,eps=1e-5,target=0,alpha=10,maxsteps=30,verbose=0):
    ...
    for scale in np.linspace(1/nStepsRamp,1,nStepsRamp):
        ...
        if not ERROR:
            ...
            if 2*nStepsRamp>100:
                raise Exception('Ramping up failed at scaling %.2f with %d ramping steps. Try different feedback parameters.'%(scale,nStepsRamp))
            else:
                print('Feedback did not succeed at scaling %.2f. Trying with %d ramping steps.'%(scale,2*nStepsRamp))
                SC = SCrampUpErrors(SC0,nStepsRamp=2*nStepsRamp,eps=eps,target=target,alpha=alpha,maxsteps=maxsteps,verbose=verbose)
                return
    return SC
