...
def SCfitInjectionZ(SC,mode,nDims=None,nBPMs=None,nShots=None,verbose=0,plotFlag=0):
    ...
    if mode == 'fitTrajectory':
        ...
        if plotFlag:
            ...
            plt.figure(342);plt.clf();titleStr=['Horizontal','Vertical']
            for nDim in range(2):
                plt.subplot(1,2,nDim+1)
                plt.plot(sBPM,1E3*Bref[nDim,:], 'O--', sBPM, 1E3*B1[nDim,nBPMs], 'X--')
                plt.xlabel('s [m]');plt.ylabel('BPM reading [mm]');plt.title(titleStr[nDim]);plt.legend(['Initial','After correction'])
            plt.show()
    elif mode == 'injectionDrift':
        tmpS = findspos(SC.RING,SC.ORD.BPM)
        sBPM = [tmpS[-1] - findspos(SC.RING,len(SC.RING)+1) , tmpS[0]]
        ...
        if plotFlag:
            ...
            for nDim in nDims:
                plt.subplot(1,2,nDim+1)
                plt.plot(sBPM,1E6*Bref[nDim,:], 'o', sBPM, 1E6*(sol[nDim][0]*sBPM+sol[nDim][1]), '--', sBPM, 1E6*(SC.INJ.Z0[2*nDim]*sBPM+SC.INJ.Z0[2*nDim-1]), 'k-', sBPM, [0,0], 'k--')
                plt.legend(['BPM reading','Fitted trajectory','Real trajectory']);plt.xlabel('s [m]');plt.ylabel('Beam offset [mm]');plt.title(titleStr[nDim])
            plt.show()
    else:
        print('Unsupported mode: ' + mode)
    if verbose:
        print('\nInjection trajectory corrected from \n x:  %.0fum -> %.0fum \n x'': %.0furad -> %.0furad \n y:  %.0fum -> %.0fum \n y'': %.0furad -> %.0furad\n' % (1E6*SC.INJ.Z0[0],1E6*(SC.INJ.Z0[0]+deltaZ0[0]),1E6*SC.INJ.Z0[1],1E6*(SC.INJ.Z0[1]+deltaZ0[1]),1E6*SC.INJ.Z0[2],1E6*(SC.INJ.Z0[2]+deltaZ0[2]),1E6*SC.INJ.Z0[3],1E6*(SC.INJ.Z0[3]+deltaZ0[3])))
