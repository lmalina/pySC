...
def calcBPMreading(SC,T,atAllElements=0):
    if SC.INJ.trackMode=='ORB':
        ...
        nParticles = SC.INJ.nParticles
    BPMoffset   = np.tile(np.array(atgetfieldvalues(SC.RING[SC.ORD.BPM],'Offset')) + np.array(atgetfieldvalues(SC.RING[SC.ORD.BPM],'SupportOffset')),(1,nTurns))
    BPMcalError = np.tile(np.array(atgetfieldvalues(SC.RING[SC.ORD.BPM],'CalError')),(1,nTurns))
    BPMroll     = np.tile(atgetfieldvalues(SC.RING[SC.ORD.BPM],'Roll'),(1,nTurns)) + np.tile(atgetfieldvalues(SC.RING[SC.ORD.BPM],'SupportRoll'),(1,nTurns))
    ...
    if nParticles > 1:
        ...
        Bx1 = np.nanmean(Tx,1)
        By1 = np.nanmean(Ty,1)
        beamLost  = np.where( np.sum( np.isnan( Tx ),1 ) * (1 + BPMsumError*SCrandnc(2,BPMsumError.shape)) > ( nParticles * SC.INJ.beamLostAt ) )[0][0]
        Bx1[beamLost:] = np.nan
        By1[beamLost:] = np.nan
    else:
        Bx1 = T[0,nE]
